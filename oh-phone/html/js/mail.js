...
$(document).on('click', '#accept-mail', function(e){
    ...
    $.post('https://oh-phone/AcceptMailButton', JSON.stringify({
...
$(document).on('click', '#remove-mail', function(e){
    e.preventDefault();
    var MailData = $("#"+OpenedMail).data('MailData');
    $.post('https://oh-phone/RemoveMail', JSON.stringify({
        mailId: MailData.mailid
    }));
    $(".mail-home").animate({
        left: 0+"vh"
...
QB.Phone.Functions.SetupMails = function(Mails) {
    ...
    $("#mail-header-mail").html(QB.Phone.Data.PlayerData.charinfo.firstname+"."+QB.Phone.Data.PlayerData.charinfo.lastname+"@NoName.com");
...
$(document).on('click', '#advert-sendmessage-chat', function(e){
    ...
    if (Advert !== "") {
        ...
        if (!picture){
            $.post('https://oh-phone/PostAdvert', JSON.stringify({
                message: Advert,
                url: null
            }));
            ClearInputNew()
...
$(document).on('click','#new-advert-photo',function(e){
    e.preventDefault();
    $.post('https://oh-phone/TakePhoto',function(url){
        if(url){
...
$(document).on('click','#adv-delete',function(e){
    e.preventDefault();
    $.post('https://oh-phone/DeleteAdvert', function(){
        setTimeout(function(){
